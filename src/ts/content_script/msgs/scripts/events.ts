    ...
    (msg: t.Msg): t.Any =>
        err(() => {
            ...
            if (['load_settings', 'load_settings_content_script'].includes(msg_str)) {
                void s_actions.Actions.run_reload_actions_2();
            } else if (msg_str === 'run_deferred_generate_server_location_url_fs') {
                void d_icons.Icons.run_deferred_generate_server_location_url_fs();
            } else if (msg_str === 'execute_command') {
                switch (msg.command) {
                    case 'enable_infinite_scrolling':
                        void d_side_panel.InfiniteScrollingState.change();
                        break;
                    case 'scroll_to_top':
